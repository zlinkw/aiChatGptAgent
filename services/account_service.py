...
class AccountService:
    ...
    def _normalize_account(self, item: dict) -> dict | None:
        ...
        normalized["phone_verified"] = normalized.get("phone_verified") or ""
        normalized["refresh_token"] = normalized.get("refresh_token") or ""
        normalized["id_token"] = normalized.get("id_token") or ""
        return normalized
    ...
    def _list_ready_candidate_tokens(self, excluded_tokens: set[str] | None = None) -> list[str]:
        ...
        strategy = str(getattr(config, "account_route_strategy", "") or "round_robin").strip()
        plus_types = {"plus", "pro", "prolite", "team"}

        all_candidates = [
            (token, (item.get("type") or "free").lower())
        ...
        if strategy == "plus_first":
            plus_tokens = [t for t, tp in all_candidates if tp in plus_types]
            return plus_tokens if plus_tokens else [t for t, _ in all_candidates]
        elif strategy == "plus_only":
            return [t for t, tp in all_candidates if tp in plus_types]
        elif strategy == "free_only":
            return [t for t, tp in all_candidates if tp not in plus_types]
        else:  # round_robin
            return [t for t, _ in all_candidates]
    ...
    def get_text_access_token(self, excluded_tokens: set[str] | None = None) -> str:
        ...
        with self._lock:
            all_candidates = [
                (token, account.get("type") or "free")
            ...
            plus_types = {"plus", "pro", "prolite", "team"}
            if strategy == "plus_first":
                plus_tokens = [t for t, tp in all_candidates if tp.lower() in plus_types]
                candidates = plus_tokens if plus_tokens else [t for t, _ in all_candidates]
            elif strategy == "plus_only":
                candidates = [t for t, tp in all_candidates if tp.lower() in plus_types]
            elif strategy == "free_only":
                candidates = [t for t, tp in all_candidates if tp.lower() not in plus_types]
            else:  # round_robin
                candidates = [t for t, _ in all_candidates]
