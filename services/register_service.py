...
def _normalize(raw: dict) -> dict:
    ...
    cfg["proxy_pool"] = cfg.get("proxy_pool") if isinstance(cfg.get("proxy_pool"), dict) else {}
    cfg["enabled"] = bool(cfg.get("enabled"))
    cfg["cpa_export"] = cfg.get("cpa_export") if isinstance(cfg.get("cpa_export"), dict) else {"base_url": "", "secret_key": ""}
    cfg["sms"] = cfg.get("sms") if isinstance(cfg.get("sms"), dict) else {"base_url": "", "codes": []}
    if "base_url" not in cfg["sms"]:
        cfg["sms"]["base_url"] = ""
    if "codes" not in cfg["sms"]:
        cfg["sms"]["codes"] = []
...
class RegisterService:
    ...
    def update(self, updates: dict) -> dict:
        with self._lock:
            ...
            openai_register.config.update({k: self._config[k] for k in ("mail", "proxy", "proxy_pool", "total", "threads", "cpa_export", "sms") if k in self._config})
            self._save()
            return self.get()
