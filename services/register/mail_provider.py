...
class CloudflareTempMailProvider(BaseMailProvider):
    ...
    def __init__(self, entry: dict, conf: dict):
        ...
        self.random_subdomain = bool(entry.get("random_subdomain", False))
        self.session = curl_requests.Session(impersonate="chrome")

    @staticmethod
    def _resolve_domain(domain: str, random_subdomain: bool) -> str:
        """解析域名，支持通配符和随机子域名。"""
        text = str(domain or "").strip().lower()
        if text.startswith("*.") and len(text) > 2:
            return f"{_random_subdomain_label()}.{text[2:]}"
        if random_subdomain:
            return f"{_random_subdomain_label()}.{text}"
        return text

    @staticmethod
    def _generate_username(random_subdomain: bool) -> str:
        """生成邮箱用户名。
        
        random_subdomain=True 时生成 edu.随机.xyz 格式的用户名。
        否则生成普通的随机用户名。
        """
        if random_subdomain:
            return f"edu.{_random_subdomain_label()}.xyz"
    ...
    def create_mailbox(self, username: str | None = None) -> dict[str, Any]:
        name = username or self._generate_username(self.random_subdomain)
        domain = _next_domain(self.domain)
        # random_subdomain 模式下不用 enablePrefix，因为用户名已经包含完整格式
        use_prefix = not self.random_subdomain
        data = self._request("POST", "/admin/new_address", headers={"x-admin-auth": self.admin_password}, payload={"enablePrefix": use_prefix, "name": name, "domain": domain})
