...
type SettingsStore = {
  ...
  setRegisterProxyPool: (field: string, value: string | boolean) => void;
  ...
  setRegisterCpaExport: (field: "base_url" | "secret_key", value: string) => void;
  setRegisterSmsCodes: (value: string) => void;
  setRegisterSmsBaseUrl: (value: string) => void;
...
export const useSettingsStore = create<SettingsStore>((set, get) => ({
  ...
  setRegisterProxyPool: (field, value) => {
    set((state) => {
      if (!state.registerConfig) return {};
      const current = state.registerConfig.proxy_pool || {
        enabled: false, mode: "userpass", host: "", port: "", username: "", password: "",
        protocol: "http", session_prefix: "session-", extra_params: "",
        api_url: "", api_protocol: "http", api_refresh_seconds: 300,
      };
      return { registerConfig: { ...state.registerConfig, proxy_pool: { ...current, [field]: value } } };
    });
  },
  ...
  setRegisterCpaExport: (field, value) => {
    set((state) => {
      if (!state.registerConfig) return {};
      const current = state.registerConfig.cpa_export || { base_url: "", secret_key: "" };
      return { registerConfig: { ...state.registerConfig, cpa_export: { ...current, [field]: value } } };
    });
  },

  setRegisterSmsCodes: (value) => {
    set((state) => {
      if (!state.registerConfig) return {};
      const codes = value.split(/[\n,]/).map((c) => c.trim()).filter(Boolean);
      const current = state.registerConfig.sms || { base_url: "", codes: [] };
      return { registerConfig: { ...state.registerConfig, sms: { ...current, codes } } };
    });
  },

  setRegisterSmsBaseUrl: (value) => {
    set((state) => {
      if (!state.registerConfig) return {};
      const current = state.registerConfig.sms || { base_url: "", codes: [] };
      return { registerConfig: { ...state.registerConfig, sms: { ...current, base_url: value } } };
    });
  },
  ...
  saveRegister: async () => {
    ...
    try {
      ...
      const data = await updateRegisterConfig({
        ...
        proxy_pool: registerConfig.proxy_pool,
        ...
        cpa_export: registerConfig.cpa_export,
        sms: registerConfig.sms,
      } as Parameters<typeof updateRegisterConfig>[0]);
      set({ registerConfig: data.register });
